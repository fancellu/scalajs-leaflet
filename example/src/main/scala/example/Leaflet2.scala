...
object Leaflet2 extends {
  ...
  def main(el: String, southwarkpark: js.Object): LMap = {
    ...
    val mapPoked = (e: LeafletMouseEvent) => {
      mapPopup
        .setLatLng(e.latlng)
        .setContent(s"You clicked me at ${e.latlng}")
        .openOn(lmap)
    }
    ...
    val marker = L
      .marker(
        (51.5, -0.09),
        MarkerOptions.title("I am draggable").draggable(true).opacity(0.8))
    ...
    L.polyline(points.map(t => (t._1 + 0.01, t._2 + 0.01)),
    ...
    L.popup(PopupOptions.closeButton(false))
      .setLatLng((51.5, -0.04))
    ...
    val imageOverlay =
      L.imageOverlay(url, latLngBnds, ImageOverlayOptions.interactive(true))
    ...
    val rect = L.rectangle(latLngBnds.pad(.01),
                           PolylineOptions.interactive(false).fill(false))

    val layerGroup = L.layerGroup(js.Array(imageOverlay, rect))
    layerGroup.addLayer(
      L.circleMarker((51.4863, -0.10935),
                     CircleMarkerOptions.fill(true).fillOpacity(1)))

    val leafOptions = IconOptions
    ...
    L.marker((51.52, -0.08), MarkerOptions.icon(greenIcon))
      .bindPopup("I am a green leaf.")
      .addTo(lmap)
    L.marker((51.52, -0.089), MarkerOptions.icon(redIcon))
    ...
    L.control
      .layers(basemaps,
              overLays = overlays,
              ControlLayerOptions.collapsed(false).hideSingleBase(true))
      .addTo(lmap)
